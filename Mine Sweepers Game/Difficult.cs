...
namespace Mine_Sweepers_Game
{
    public partial class Difficult : Form
    {
        ...
        private String flag = "🚩";
        private int flagged = 60;
        private int foundmines = 0;
        private int minesno = 60;
        public Difficult()
        {
            InitializeComponent();
            this.game = new NewGame(minesno, 20, this);
            int i = 19;
            int j = 19;
        ...
        void checkBoxes(Button b)
        {
            ...
            if (text != mine)
            {
                if (text == "")
                {
                    ...
                    if (j + 1 < 20 && buttons[i, j + 1].Enabled == true)
                    {
                        checkBoxes(buttons[i, j + 1]);
                    }
                    if (i + 1 < 20 && buttons[i + 1, j].Enabled == true)
                    {
                        checkBoxes(buttons[i + 1, j]);
                    }
                }
